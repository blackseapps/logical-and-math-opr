function Islem(GelenIslem) {
  ...
  while (true) {
    for (var i = 0; i < GelenVeri.length; i++) {
      ...
      if (parca_tut == "(") {
        Parantez_Yakalama = true;
        Acilan_parantez++;
        if (Acilan_parantez > 1) {
          Parantez_Ici = "";
        }
        continue;
      }
      if (parca_tut == ")") {
        Parantez_Yakalama = false;
        break;
      }
      if (Parantez_Yakalama) {
        Parantez_Ici = Parantez_Ici + parca_tut;
      }
    }
...
function TerimAyirici(Terim, ID) {
  ...
  for (i = ID + 1; i <= Terim.length - 1; i++) {
    ...
    if (Opetorler.indexOf(parca_tut) < 0) {
      veri2 = veri2 + parca_tut;
  ...
  if (
    (!isaret1 && isaret2 && veri1 < veri2) ||
    (!isaret1 && !isaret2 && Islem_turu == "*")
  ) {
    pozitif = "+";
  }
